...
signed_size_type homa_sendto(socket_type s, const buf* bufs,
    size_t count, int flags, const void* addr,
    std::size_t addrlen
...
size_t homa_sync_sendto(socket_type s, state_type state,
    const buf* bufs, size_t count, int flags, const void* addr,
                        std::size_t addrlen, uint64_t *id, uint64_t completion_cookie, boost::system::error_code& ec)
{
  ...
  {
    // Try to complete the operation without blocking.
    signed_size_type bytes = socket_ops::homa_sendto(
